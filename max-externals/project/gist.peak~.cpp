/**
	@file
	gist.peak~: a peak energy Max object
	by Adam Stark
	@ingroup examples	
*/
...
{	
	...
	t_class *c = class_new("gist.peak~", (method)gistmaxobj_new, (method)gistmaxobj_freeall, (long)sizeof(t_gistmaxobj), 0L, A_GIMME, 0);
...
void *gistmaxobj_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if (x) {
        ...
        post("gist.peak~ - by Adam Stark, based on version %s of Gist",GIST_VERSION);
	}
	return (x);
}
...
t_int *gistmaxobj_perform(t_int *w)
{
    ...
    double peak = x->tdf->peakEnergy(frame);
...
void gistmaxobj_perform64(t_gistmaxobj *x, t_object *dsp64, double **ins, long numins, double **outs, long numouts, long sampleframes, long flags, void *userparam)
{
    ...
    //----------------------------------------------
    double peak = x->tdf->peakEnergy(inL,n);
